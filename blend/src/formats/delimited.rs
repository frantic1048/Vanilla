...
/// Renderer for `key value` line pairs (ordered, allows duplicate keys).
///
/// NCL data shape: `[["key", "value"], ...]`
/// Output: `key value\n` per pair
pub struct SpacePairLinesRenderer;

impl FormatRenderer for SpacePairLinesRenderer {
    fn render(&self, value: &serde_json::Value) -> Result<String> {
        let arr = value
            .as_array()
            .ok_or_else(|| anyhow::anyhow!("SpacePairLines expects an array of pairs"))?;
...
/// Renderer for `key value` record lines (no duplicate keys).
///
/// NCL data shape: `{"key": "value", ...}` (Nickel record)
/// Output: `key value\n` per entry
pub struct SpaceRecordLinesRenderer;

impl FormatRenderer for SpaceRecordLinesRenderer {
    fn render(&self, value: &serde_json::Value) -> Result<String> {
        let obj = value
            .as_object()
            .ok_or_else(|| anyhow::anyhow!("SpaceRecordLines expects a JSON object"))?;
...
/// NCL data shape: `{"key": "value", ...}` (Nickel record)
/// Output: `key=value\n` per entry
pub struct EqualsRecordLinesRenderer;

impl FormatRenderer for EqualsRecordLinesRenderer {
    fn render(&self, value: &serde_json::Value) -> Result<String> {
        let obj = value
            .as_object()
            .ok_or_else(|| anyhow::anyhow!("EqualsRecordLines expects a JSON object"))?;
...
mod tests {
    ...
    #[test]
    fn test_render_space_pair_lines() {
        let renderer = SpacePairLinesRenderer;
        let value = json!([
            ["font_size", "16"],
    ...
    #[test]
    fn test_parse_space_pair_lines() {
        let renderer = SpacePairLinesRenderer;
    ...
    #[test]
    fn test_render_space_record_lines() {
        let renderer = SpaceRecordLinesRenderer;
        let value = json!({
            "font_size": "14",
    ...
    #[test]
    fn test_parse_space_record_lines() {
        let renderer = SpaceRecordLinesRenderer;
        let input = "# Comment\nfont_size 14\nfont_family JetBrains Mono\n";
    ...
    #[test]
    fn test_render_equals_record_lines() {
        let renderer = EqualsRecordLinesRenderer;
        let value = json!({
    ...
    #[test]
    fn test_parse_equals_record_lines() {
        let renderer = EqualsRecordLinesRenderer;
        let input = "# Comment\nprefix=~/.local/share/npm\nsave-exact=true\n";
